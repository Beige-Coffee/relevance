...
export function GraphLegend({ mode }: { mode: GraphMode }) {
  ...
  return (
    <div className="absolute bottom-5 right-5 z-10">
      <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)]/95 backdrop-blur shadow-sm">
        <button
        ...
        {open && (
          <div className="px-3 pb-3 space-y-1.5 text-xs text-[var(--ink-soft)]">
            ...
            {mode === "concepts" ? (
              <p className="text-[11px] leading-snug">Lines connect related, prerequisite, and contrasted concepts.</p>
            ) : (
              <p className="text-[11px] leading-snug">Lines connect thinkers Vervaeke discusses together. Thicker means more shared episodes and concepts.</p>
            )}
          </div>
        )}
      </div>
